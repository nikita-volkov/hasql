    ...
    describe "SessionError types" do
      describe "QueryError" do
        describe "with ClientError" do
          it "reproduces ClientError without message" \connection -> do
        ...
        describe "with ResultError subtypes" do
          ...
          describe "UnexpectedResult" do
            it "reproduces UnexpectedResult error" \connection -> do
          ...
          describe "CellError with ValueError" do
            ...
            it "reproduces DecoderTypeMismatch for invalid timestamp format" \connection -> do
              let statement = Statement.Statement 
                    "SELECT 'not-a-timestamp'" 
                    Encoders.noParams 
    ...
    describe "PipelineError" do
      ...
      describe "with CellError" do
        it "reproduces DecoderTypeMismatch for NULL in pipeline" \connection -> do
          result <- Connection.use connection do
            Session.pipeline do
              Pipeline.statement () (Statement.Statement 
                "SELECT NULL" 
                Encoders.noParams 
                (Decoders.singleRow (Decoders.column (Decoders.nonNullable Decoders.int4)))
                False)
