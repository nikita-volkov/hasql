module Sharing.ByUnit.Hipq.SessionSpec (spec) where
...
withConnection (host, port) action = do
  let connectionString = buildConnectionString host port
  bracket
    (Pq.connectdb connectionString)
    (\connection -> Pq.finish connection)
    (\connection -> do
...
buildConnectionString :: Text -> Word16 -> ByteString
