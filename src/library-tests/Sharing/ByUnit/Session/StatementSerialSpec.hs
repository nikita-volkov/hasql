module Sharing.ByUnit.Session.StatementSerialSpec (spec) where
  ...
  describe "Roundtrips" do
    it "handles simple values correctly" \config -> do
      Scripts.onPreparableConnection config \connection -> do
        result <- Connection.use connection (Session.statementSerial (42 :: Int64) echoStatement)
      ...
      Scripts.onPreparableConnection config \connection -> do
        result <-
          Connection.use connection do
            a <- Session.statementSerial (1 :: Int64) echoStatement
            b <- Session.statementSerial (2 :: Int64) echoStatement
            pure (a, b)
    ...
    it "keeps a prepared statement usable after an EXECUTE error" \config -> do
      ...
      Scripts.onPreparableConnection config \connection -> do
        failure <- Connection.use connection (Session.statementSerial 0 divStatement)
        failure `shouldSatisfy` isLeft
        success <- Connection.use connection (Session.statementSerial 1 divStatement)
    ...
    it "matches the result of Session.statement" \config -> do
      Scripts.onPreparableConnection config \connection -> do
        viaSerial <- Connection.use connection (Session.statementSerial (7 :: Int64) echoStatement)
        viaPipeline <- Connection.use connection (Session.statement (7 :: Int64) echoStatement)
        viaSerial `shouldBe` viaPipeline
  ...
  describe "Error Handling" do
    it "captures query errors correctly" \config -> do
      Scripts.onPreparableConnection config \connection -> do
        ...
        result <- Connection.use connection (Session.statementSerial ([3, 7] :: [Int64], "a") statement)
        case result of
