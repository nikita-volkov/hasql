...
newtype SelectTypeInfo = SelectTypeInfo
  { -- | Set of (schema name, type name) pairs to look up.
    keys :: HashSet CodecsCore.QualifiedTypeName
  }

-- | Result maps (schema name, type name) pairs to TypeInfo (scalar OID, array OID).
type SelectTypeInfoResult =
  HashMap CodecsCore.QualifiedTypeName CodecsCore.TypeInfo.TypeInfo
...
encodeParams (SelectTypeInfo keys) =
  let (schemaNames, typeNames) = unzip (fmap CodecsCore.QualifiedTypeName.toNameTuple (HashSet.toList keys))
...
decoder =
  ...
  where
    step acc (schemaName, typeName, typeOid, arrayOid) =
      HashMap.insert (CodecsCore.QualifiedTypeName.QualifiedTypeName schemaName typeName) (CodecsCore.TypeInfo.TypeInfo typeOid arrayOid) acc
