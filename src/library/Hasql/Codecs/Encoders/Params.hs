        ...
        Just oid ->
          Params
            { size,
              ...
              columnsMetadata = pure (TypeShape (CodecsVocab.TypeRef.KnownOid oid) dimensionality textFormat),
              printer
            }
        Nothing ->
          let key = CodecsVocab.QualifiedTypeName.QualifiedTypeName schemaName typeName
           in Params
                { size,
                  request = toRequest (ToBeResolved.lookup key *> serialize),
                  columnsMetadata = pure (TypeShape (CodecsVocab.TypeRef.NamedType key) dimensionality textFormat),
                  printer
                }
