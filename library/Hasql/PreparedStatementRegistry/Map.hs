...
data RegistryState = RegistryState !(HM.HashMap LocalKey ByteString) !Word

-- | Create an empty registry state
{-# INLINEABLE empty #-}
empty :: RegistryState
empty = RegistryState HM.empty 0

-- | Pure lookup operation
{-# INLINEABLE lookup #-}
lookup :: LocalKey -> RegistryState -> Maybe ByteString
lookup localKey (RegistryState hashMap _) = HM.lookup localKey hashMap
...
reset _ = RegistryState HM.empty 0
